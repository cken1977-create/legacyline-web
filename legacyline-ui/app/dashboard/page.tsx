  ...
  try {
    ...
    return all.filter(
      (p) =>
        ...
        p.first_name &&
        p.first_name.trim() !== "" &&
        p.organization === "vizionz_sankofa"
    );
  } catch {
    return [];
  }
    ...
    <div className="rounded-2xl bg-black/30 p-5 ring-1 ring-white/10">
      ...
      <div
        className={`mt-2 text-lg font-semibold tracking-tight ${
          accent === "gold" ? "text-[#C8A84B]" : "text-white"
        }`}
      >
        {value}
      </div>
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; classes: string }> = {
    registered: {
      label: "Registered",
      classes: "bg-blue-500/15 text-blue-400 ring-blue-500/25",
    },
    data_collecting: {
      label: "Data Collection",
      classes: "bg-yellow-500/15 text-yellow-400 ring-yellow-500/25",
    },
    under_review: {
      label: "Under Review",
      classes: "bg-orange-500/15 text-orange-400 ring-orange-500/25",
    },
    evaluated: {
      label: "Evaluated",
      classes: "bg-purple-500/15 text-purple-400 ring-purple-500/25",
    },
    certified: {
      label: "Certified",
      classes: "bg-emerald-500/15 text-emerald-400 ring-emerald-500/25",
    },
    revoked: {
      label: "Revoked",
      classes: "bg-red-500/15 text-red-400 ring-red-500/25",
    },
  };
  const config = map[status] ?? {
    label: status,
    classes: "bg-white/10 text-white/60 ring-white/10",
  };
  return (
    <span
      className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ring-1 ${config.classes}`}
    >
      {config.label}
    </span>
  );
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-white/55">
                Pilot Partner · Individual Readiness Program
              </p>
            </div>
          ...
          <div className="rounded-3xl bg-white/5 ring-1 ring-white/10 overflow-hidden">
            <div className="p-7 pb-4">
              ...
              <p className="mt-1 text-xs text-white/45">
                ...
                {participants.length !== 1 ? "s" : ""} in system
              </p>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-white/5">
                  {participants.map((p) => (
                    ...
                    >
                      <td className="px-7 py-4 font-mono text-xs text-white/60">
                        {p.subject_number ?? "—"}
                      </td>
